...
class JWT
{
	...
	public static $tiempoMaxEx = 3600;
    ...
    public static function decode($jwt, $key, $allowed_algs = array())
    {
        ...
        if (isset($payload->exp) && (time() - self::$leeway) >= $payload->exp) {
			...
			throw new Exception('Su consulta a expirado',$payload->ses);
        }
		
		// verificando que el tiempo de expiracion no sea muy grande
        if ( $payload->iat + self::$tiempoMaxEx < $payload->exp) {
            //throw new ExpiredException('Expired token');
			throw new Exception('El tiempo de expiracion supera el establecido, no intente hacer esto',$payload->ses);
        }
